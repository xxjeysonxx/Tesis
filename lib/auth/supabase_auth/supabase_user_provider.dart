...
class AppReportesCDRSupabaseUser extends BaseAuthUser {
  AppReportesCDRSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> appReportesCDRSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppReportesCDRSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
